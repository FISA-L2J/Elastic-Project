...
- 브랜드 충성도와 소비 데이터 동시 확보 가능
- 특정 소비 카테고리 Lock-in 가능
...
* **결측치 제거**: 소비 금액/연령 정보가 없는 데이터 제외
* **이상치 처리**: 비정상적으로 큰 단일 결제 금액 제거
...
프로젝트 진행 과정에서 발생한 기술적 문제와 이를 극복한 과정을 정리했습니다.

### 🔍 Issue 1. 소비 카테고리 분류의 정확성 한계
* **발생 배경:** 가맹점 소비 데이터를 카테고리별로 분류하는 과정에서
일부 가맹점의 업종 정보가 불명확하거나 중복되는 문제가 발생
* **원인 분석:** 카드사 가맹점 업종 코드가 실제 소비 행태를 완전히 반영하지 못하며,
동일 가맹점이라도 복수의 소비 목적을 가지는 경우가 존재
* **해결 방법:** 유사 소비 항목을 상위 카테고리로 통합, 해석에 혼선을 줄 수 있는 소수 항목은 분석 대상에서 제외
* **결과 및 성과:** 소비 패턴 분석 시 불필요한 노이즈를 줄였으며,
연령대별 주요 소비 특성이 보다 직관적으로 드러남
